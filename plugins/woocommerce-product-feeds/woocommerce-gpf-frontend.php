...
class WoocommerceGpfFrontend {
	...
	private function process_product($post) {
        ...
        if (empty($this->settings['include_variations']) ||
            ...
            $toreturn = $this->process_simple_product($post, $woocommerce_product);
        } elseif ($woocommerce_product->is_type('variable')) {
            $toreturn = $this->process_variable_product($post, $woocommerce_product);
        }
        ...
        foreach ($taxonomies as $taxonomy) {
            if (empty($this->settings['include_variations']) ||
                ...
                $toreturn =  $this->process_simple_product($post, $woocommerce_product,$taxonomy);
            } elseif ($woocommerce_product->is_type('variable')) {
                $toreturn =  $this->process_variable_product($post, $woocommerce_product,$taxonomy);
            }
        }
	 ...
	 *                                       otherwise.
	 */
	private function process_simple_product( $post, $woocommerce_product, $taxonomy = null ) {
		$feed_item = new stdClass();
		$this->get_product_prices( $feed_item, $woocommerce_product );

		// Get main item information
		$feed_item->ID = $post->ID;
		$feed_item->guid = 'woocommerce_gpf_' . rand(100,999) . '_' . $post->ID;
